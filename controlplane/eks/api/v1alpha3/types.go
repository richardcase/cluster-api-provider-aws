...
type OIDCProviderConfig struct {
	...
	// IdentityProviderConfigName is the name of the OIDC provider configuration
	// +kubebuilder:validation:MinLength:=2
	// +kubebuilder:validation:Required
	IdentityProviderConfigName string `json:"identityProviderConfigName"`

	// IssuerURL is the URL of the OIDC provider that allows the API server to discover
	...
	// GroupsClaim is the JWT claim that is used to return your groups
	// +optional
	GroupsClaim string `json:"groupsClaim"`

	// GroupsPrefix is the prefix that is prepended to group claims to prevent clashes with existing
	...
	// RequiredClaims are the key value pairs that describe required claims in the identity token.
	...
	// UsernameClaim is the JWT claim to use as the username. The default is sub,
	...
	// UsernamePrefix is the prefix that is prepended to username claims to prevent clashes with existing
